use ml_dsa_wallet_tools::{
    cast_abi_encode, cast_call, cast_call_from, cast_calldata, cast_keccak, cast_send, chain_id,
    ensure_0x, flag_value, has_flag, latest_block_timestamp, load_json, state_dir, string_field,
    strip_0x, Result,
};
...
fn main() -> Result<()> {
    ...
    let note =
        flag_value(&args, "--note").unwrap_or_else(|| "hello from ML-DSA compat wallet".to_string());
    ...
    let expected_public_key = cast_call(&rpc_url, wallet, "publicKey()(bytes)", &[])?;
    let local_public_key = ensure_0x(string_field(&key_data, "public_key_hex")?);
    if expected_public_key.to_lowercase() != local_public_key.to_lowercase() {
        return Err("ml_dsa_keypair.json does not match wallet publicKey()".into());
    }
    let payer = cast_call(&rpc_url, wallet, "payer()(address)", &[])?;
    let nonce = cast_call(&rpc_url, wallet, "nonce()(uint256)", &[])?.parse::<u64>()?;
    ...
    let encoded = cast_abi_encode(
        "f(uint256,address,uint256,address,uint256,bytes32,uint256)",
        &[
    ...
    let call_result = cast_call_from(
        &rpc_url,
        wallet,
        Some(&payer),
        "execute(address,uint256,bytes,uint256,bytes)(bytes)",
        &[
            target.to_string(),
            value.to_string(),
    ...
    let updated_nonce = cast_call(&rpc_url, wallet, "nonce()(uint256)", &[])?;
    println!("Updated wallet nonce: {}", updated_nonce);
